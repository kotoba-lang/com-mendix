(ns mendix.main
  "Kotodama WASM entrypoint for the mendix clean-room actor (L4) — Clojure port.
  ...
  facts (`mendix.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mendix")
(def tier "L4")
...
(def entity-specs
  [{:entity "Flow"       :plural "flows"       :id-prefix "mendix_flo"
    :fields [:name :trigger :status]           :required [:name :trigger]
    :coerce {}                        :refs {}}
   {:entity "Step"       :plural "steps"       :id-prefix "mendix_ste"
    :fields [:flowId :appName :action :position] :required [:appName :action]
    :coerce {:position :int}          :refs {:flowId "Flow"}}
   {:entity "Connection" :plural "connections" :id-prefix "mendix_con"
    :fields [:appName :authType :active]       :required [:appName :authType]
    :coerce {:active :bool}           :refs {}}
   {:entity "Run"        :plural "runs"        :id-prefix "mendix_run"
    :fields [:flowId :status :durationMs :startedAt] :required [:status :durationMs]
    :coerce {:durationMs :int}        :refs {:flowId "Flow"}}
   {:entity "App"        :plural "apps"        :id-prefix "mendix_app"
    :fields [:name :category]                  :required [:name :category]
    :coerce {}                        :refs {}}
   {:entity "Webhook"    :plural "webhooks"    :id-prefix "mendix_web"
    :fields [:flowId :url :active]             :required [:url :active]
    :coerce {:active :bool}           :refs {:flowId "Flow"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"mendix.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mendix-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mendix-compat/ready)
